...
class INFECTOR:
    ...
    def create_dicts(self):
        ...
        """
#        f = open(self.fn+"/train_set.txt","r") # CHANDU: Updated
    ...
    def model(self):
        ...
        with self.graph.as_default():
            ...
            nce_biases = tf.Variable(tf.zeros([self.target_size])) # CHADNU: EDITED tf.Variable(tf.zeros([self.vocabulary_size]))
            gamma = 1.0
            self.loss1 = gamma * tf.reduce_mean(
                tf.nn.nce_loss(weights= T,
                     biases=nce_biases,
                     labels=v,
                     inputs=Su,
                     num_sampled=self.num_samples,
                     num_classes = self.target_size))# CHADNU: EDITED num_classes=self.vocabulary_size))
    ...
    def train(self):
        ...
        with  tf.Session(graph = self.graph) as sess:
            ...
            for epoch in range(self.n_epochs):
                 #--------- Train 
                 f = open(self.fn.capitalize()+"/Init_Data/train_set.txt","r") # CHANDU: Updated
                 idx = 0
                 init= -1
                 inputs = []
                 labels = []
                 ...
                 for line in f:
					...
					#---- check if we are at the same cascade
                      if(init==original or init<0):
                          ...
                          fairness_score = float(float(sample[3]) * 1) # CHANDU: EDITED FOR ANALYSIS
