...
class IMINFECTOR:
    def __init__(self, fn, embedding_size, seed_size, final_seeds_filename):
        self.fn = fn
        self.embedding_size = embedding_size
        self.file_Sn = fn.capitalize()+"/Embeddings/infector_source3.txt" # CHANDU: UPDATED
        self.file_Tn = fn.capitalize()+"/Embeddings/infector_target3.txt"# CHANDU: UPDATED
        self.size = seed_size # CHANDU: UPDATED
        self.P = 10
        self.final_seeds_filename = final_seeds_filename # CHANDU: UPDATED
#         if(fn=="digg"):
#             self.size=50
#             self.P = 40
#         elif(fn=="weibo"):
...
#         norm = np.multiply(norm, influencer_fairness_array)        
    ...
    def run_method(self,init_idx):
        ...
        # Do not sort
#         ftp = open(self.fn.capitalize()+"/Seeds/final_seeds.txt","w") # CHANDU: UPDATED
        ftp = open(self.final_seeds_filename, "w")    # CHANDU: UPDATED    
...
def run(fn, embedding_size, log, seed_size, final_seeds_filename):
    f = open(fn.capitalize()+"/Init_Data/train_set.txt","r")
    start = time.time()
    iminfector = IMINFECTOR(fn, embedding_size, seed_size, final_seeds_filename)
    ...
    iminfector.compute_D(S,T,nodes_idx,init_idx, influencer_fairness_array) # CHANDU: EXPERIMENT
    del T,S,nodes_idx
