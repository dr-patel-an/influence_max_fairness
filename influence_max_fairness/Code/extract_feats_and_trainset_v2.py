...
def store_samples(fn,cascade_nodes,cascade_times,initiators,train_set,op_time, user_gender_dictionary, N_males, N_females, beta, sampling_perc=200):
    """
...
#     male_frac = len(male_list)  / ( len(male_list) +  len(female_list) )
    ...
    #========================= APPROACH 2 ===========================#       
    #max_casc_len = 6800
    #obj_metric = (0.2 * (casc_len/max_casc_len)) + (0.8 * fainess_score)
    ...
    if s_times==0:
        ...
        samples = np.random.choice(a=cascade_nodes, size=int(no_samples*(fainess_score**beta)), p=probs) 
        #samples = np.random.choice(a=cascade_nodes, size=int(no_samples*obj_metric), p=probs)
...
def run(fn,sampling_perc,log, beta):    
    print("Reading the network")
    g = ig.Graph.Read_Ncol(fn.capitalize()+"/Init_Data/"+fn+"_network.txt") ## CHANDU: Re-Updated
    ...
    f = open(fn.capitalize()+"/Init_Data/train_cascades.txt","r")  # CHANDU: Re-Edited
...
#                 influencer_cascades_dict[op_id] = influencer_cascades_dict[op_id] + cascade_nodes
            ...
            if(len(cascade_nodes)<2):
                continue
            initiators = [op_id]
        store_samples(fn,cascade_nodes[1:],cascade_times[1:],initiators,train_set,op_time,user_gender_dictionary, 
                      N_males, N_females, beta, sampling_perc)
...
#         cascade_unique = set(influencer_cascades_dict[key])
#         male_list_cascade = []
#         female_list_cascade = []
...
#         fainess_score =  fairness_score(len(male_list_cascade), len(female_list_cascade), N_males, N_females)
#         influencer_fairness.write(str(op_id)+","+ str(fainess_score) + "\n")    
...
#    graph = pd.read_csv(fn+"/"+fn+"_network.txt",sep=" ")
    graph = pd.read_csv(fn.capitalize()+"/Init_Data/"+fn+"_network.txt" ,sep=" ") # CHANDU: Updated    
...
#    f= open(fn+"/"+fn+"_incr_dic.json","w")
    f= open(fn.capitalize()+"/Init_Data/"+fn+"_incr_dic.json","w")   # CHANDU: Updated 
    json.dump(dic,f)
    f.close()
