...
test_cascade_path = "../Data/Weibo/Init_Data/train_cascades.txt"
...
influencer_infectors_dict = {}
with open(test_cascade_path) as f:
    for line in f:
        cascade = line.split(";")
        op_id = cascade[1].split(" ")[0]
        cascade_values = list(map(lambda x: x.split(" ")[0],cascade[2:]))
        if op_id in influencer_infectors_dict:
            influencer_infectors_dict[op_id] = influencer_infectors_dict[op_id] + cascade_values
        else:
            influencer_infectors_dict[op_id] = cascade_values

def get_metrics(seed_file_path, influencer_infectors_dict):
    ...
    for k, v in seed_cascades.items():
        if k in influencer_infectors_dict:
            seed_cascades[k].append(influencer_infectors_dict[k])
            seed_set.add(k)
...
# print(get_metrics('../Data/Weibo/Seeds/final_seeds.txt', influencer_infectors_dict))
...
for seed_file in all_seed_files:  
    print('\n\n============================={}============================='.format(seed_file))
    total_infected_in_cascades, total_males_in_casc, total_females_in_cas, males_fraction_in_casc, females_fraction_in_casc, fairness_score_value = get_metrics(seed_file, influencer_infectors_dict)
...
df.to_csv('./train_cascade_metrics_Approach_B.csv', index=False)
